...
alias init-devserver='ssh-keygen -f "$HOME/.ssh/known_hosts" -R "islasgeci.dev" && ssh-keyscan "islasgeci.dev" >> "$HOME/.ssh/known_hosts" && scp -pr ~/.vault evaro@islasgeci.dev:/home/evaro/.vault && ansible-playbook ~/repositorios/thin_client/ansible/development.yml --limit islasgeci.dev && devserver'
...
alias run-ci-no-compose='docker run --env BITBUCKET_EMAIL --env BITBUCKET_API_TOKEN --interactive --name ${PWD##*/}_ci --rm --tty --volume ${PWD}:/workdir islasgeci/${PWD##*/}:latest bash'
alias run-ci='docker compose run --env BITBUCKET_EMAIL --env BITBUCKET_API_TOKEN --interactive --name ${PWD##*/}_ci --rm --tty islasgeci bash'
alias run-dev='docker compose run --env BITBUCKET_EMAIL --env BITBUCKET_API_TOKEN --interactive --name ${PWD##*/}_dev --rm --tty develop bash'
